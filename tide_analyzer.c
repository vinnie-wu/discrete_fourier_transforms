/*
	...
	Purpose:      Processes a formatted tide measurement file and
	determines if the tide is once-daily, or 
	twice-daily using fast DFT.

	- The tide measurement file is a txt file whose name
	corresponds to the name defined in the preprocessor
	directive.  

	- It is a series of NUMBER_OF_READINGS tidal
...
#define NUMBER_OF_READINGS  131072 // 2^17
...
/* If ERROR occurs upon closure, increase the size
...
int main(void)
{
	...
	int i;                  /* Helpful iterator variable */
	...
	fft(DIR, EXPONENT, omega, complex_component);
	...
	*/
	for (i = 0; i < NUMBER_OF_READINGS / 2; i++) {
		if (omega[i] >= NOISE_FILTER) {
			if (omega[i] >= frequency) {
				frequency = omega[i];
				amplitude = 1;
			}

			else if (omega[i] == frequency)
				amplitude++;
		}
	}
	...
	*/
	...
	fprintf(file_pointer, "%s %f %s", "Puddlejump tidal frequency:", frequency, "per day.");

	/* Closes the result file */
	if (file_pointer == NULL) {
		printf("Error making result.txt");
	}
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
	...
	while (fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
		...
		values_per_line = sscanf_s(line_buffer, "%d", extracted_values);
